...
public class RetriesTest : TestBase
{
    ...
    public async Task ImmediateSuccess_Works()
    {
        ...
        IncreaseClient client = new() { HttpClient = httpClient, MaxRetries = 2 };
    ...
    [Fact]
    public async Task IdempotencyHeader_Works()
    {
        ...
        handlerMock
            ...
            .Returns<HttpRequestMessage, CancellationToken>(
                (req, _) =>
                {
                    headerValue ??= Enumerable.Single(req.Headers.GetValues("Idempotency-Key"));
            ...
            .Verify(
                ...
                ItExpr.Is<HttpRequestMessage>(
                    (req) =>
                        ...
                        && req.RequestUri == new Uri("http://localhost/something")
                        && Enumerable.Single(req.Headers.GetValues("Idempotency-Key"))
                            == headerValue
                ),
                ItExpr.IsAny<CancellationToken>()
            );
    ...
    public async Task RetryAfterMsHeader_Works()
    {
        ...
        IncreaseClient client = new() { HttpClient = httpClient, MaxRetries = 1 };
