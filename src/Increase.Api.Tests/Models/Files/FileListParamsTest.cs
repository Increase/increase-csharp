...
public class FileListParamsTest : TestBase
{
    ...
    public void FieldRoundtrip_Works()
    {
        var parameters = new FileListParams
        {
            ...
            IdempotencyKey = "x",
            Limit = 1,
            Purpose = new() { In = [In.CardDisputeAttachment] },
        };
        ...
        FileListParamsPurpose expectedPurpose = new() { In = [In.CardDisputeAttachment] };
    ...
    public void Url_Works()
    {
        ...
        Assert.True(
            TestBase.UrisEqual(
                new Uri(
                    "https://api.increase.com/files?created_at.after=2019-12-27T18%3a11%3a19.117%2b00%3a00&created_at.before=2019-12-27T18%3a11%3a19.117%2b00%3a00&created_at.on_or_after=2019-12-27T18%3a11%3a19.117%2b00%3a00&created_at.on_or_before=2019-12-27T18%3a11%3a19.117%2b00%3a00&cursor=cursor&idempotency_key=x&limit=1&purpose.in=card_dispute_attachment"
                ),
                url
            )
        );
...
public class FileListParamsPurposeTest : TestBase
{
    [Fact]
    public void FieldRoundtrip_Works()
    {
        var model = new FileListParamsPurpose { In = [In.CardDisputeAttachment] };

        List<ApiEnum<string, In>> expectedIn = [In.CardDisputeAttachment];
    ...
    public void SerializationRoundtrip_Works()
    {
        ...
        string json = JsonSerializer.Serialize(model, ModelBase.SerializerOptions);
        var deserialized = JsonSerializer.Deserialize<FileListParamsPurpose>(
            json,
            ModelBase.SerializerOptions
        );
    ...
    public void OptionalNonNullablePropertiesUnsetAreNotSet_Works()
    {
        var model = new FileListParamsPurpose { };
    ...
    public void CopyConstructor_Works()
    {
        ...
        FileListParamsPurpose copied = new(model);
...
{
    [Theory]
    [InlineData(In.CardDisputeAttachment)]
    [InlineData(In.CheckImageFront)]
    [InlineData(In.CheckImageBack)]
    [InlineData(In.ProcessedCheckImageFront)]
    [InlineData(In.ProcessedCheckImageBack)]
    [InlineData(In.MailedCheckImage)]
    [InlineData(In.CheckAttachment)]
    [InlineData(In.CheckVoucherImage)]
    [InlineData(In.CheckSignature)]
    [InlineData(In.InboundMailItem)]
    [InlineData(In.Form1099Int)]
    [InlineData(In.Form1099Misc)]
    [InlineData(In.FormSS4)]
    [InlineData(In.IdentityDocument)]
    [InlineData(In.IncreaseStatement)]
    [InlineData(In.LoanApplicationSupplementalDocument)]
    [InlineData(In.Other)]
    [InlineData(In.TrustFormationDocument)]
    [InlineData(In.DigitalWalletArtwork)]
    [InlineData(In.DigitalWalletAppIcon)]
    [InlineData(In.PhysicalCardFront)]
    [InlineData(In.PhysicalCardBack)]
    [InlineData(In.PhysicalCardCarrier)]
    [InlineData(In.DocumentRequest)]
    [InlineData(In.EntitySupplementalDocument)]
    [InlineData(In.Export)]
    [InlineData(In.FeeStatement)]
    [InlineData(In.UnusualActivityReportAttachment)]
    [InlineData(In.DepositAccountControlAgreement)]
    [InlineData(In.ProofOfAuthorizationRequestSubmission)]
    [InlineData(In.AccountVerificationLetter)]
    [InlineData(In.FundingInstructions)]
    [InlineData(In.HoldHarmlessLetter)]
