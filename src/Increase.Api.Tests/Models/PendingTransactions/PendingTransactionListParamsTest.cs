...
public class PendingTransactionListParamsTest : TestBase
{
    [Fact]
    public void FieldRoundtrip_Works()
    {
        var parameters = new PendingTransactionListParams
        {
            AccountID = "account_id",
            Category = new() { In = [In.AccountTransferInstruction] },
            CreatedAt = new()
            {
            ...
            Limit = 1,
            RouteID = "route_id",
            Status = new() { In = [StatusIn.Pending] },
        };

        string expectedAccountID = "account_id";
        Category expectedCategory = new() { In = [In.AccountTransferInstruction] };
        ...
        Status expectedStatus = new() { In = [StatusIn.Pending] };
    ...
    public void Url_Works()
    {
        ...
        Assert.True(
            TestBase.UrisEqual(
                new Uri(
                    "https://api.increase.com/pending_transactions?account_id=account_id&category.in=account_transfer_instruction&created_at.after=2019-12-27T18%3a11%3a19.117%2b00%3a00&created_at.before=2019-12-27T18%3a11%3a19.117%2b00%3a00&created_at.on_or_after=2019-12-27T18%3a11%3a19.117%2b00%3a00&created_at.on_or_before=2019-12-27T18%3a11%3a19.117%2b00%3a00&cursor=cursor&limit=1&route_id=route_id&status.in=pending"
                ),
                url
            )
        );
    ...
    public void CopyConstructor_Works()
    {
        ...
        PendingTransactionListParams copied = new(parameters);
...
public class CategoryTest : TestBase
{
    ...
    public void FieldRoundtrip_Works()
    {
        var model = new Category { In = [In.AccountTransferInstruction] };

        List<ApiEnum<string, In>> expectedIn = [In.AccountTransferInstruction];
...
{
    [Theory]
    [InlineData(In.AccountTransferInstruction)]
    [InlineData(In.AchTransferInstruction)]
    [InlineData(In.CardAuthorization)]
    [InlineData(In.CheckDepositInstruction)]
    [InlineData(In.CheckTransferInstruction)]
    [InlineData(In.FednowTransferInstruction)]
    [InlineData(In.InboundFundsHold)]
    [InlineData(In.UserInitiatedHold)]
    [InlineData(In.RealTimePaymentsTransferInstruction)]
    [InlineData(In.WireTransferInstruction)]
    ...
    [InlineData(In.UkFasterPaymentSystemTransferInstruction)]
    [InlineData(In.SepaInstantTransferInstruction)]
    [InlineData(In.Other)]
...
public class StatusTest : TestBase
{
    ...
    public void FieldRoundtrip_Works()
    {
        var model = new Status { In = [StatusIn.Pending] };

        List<ApiEnum<string, StatusIn>> expectedIn = [StatusIn.Pending];
...
public class StatusInTest : TestBase
{
    [Theory]
    [InlineData(StatusIn.Pending)]
    [InlineData(StatusIn.Complete)]
    public void Validation_Works(StatusIn rawValue)
    {
        // force implicit conversion because Theory can't do that for us
        ApiEnum<string, StatusIn> value = rawValue;
        value.Validate();
    }
    ...
    public void InvalidEnumValidationThrows_Works()
    {
        var value = JsonSerializer.Deserialize<ApiEnum<string, StatusIn>>(
            JsonSerializer.SerializeToElement("invalid value"),
            ModelBase.SerializerOptions
        );
    ...
    [InlineData(StatusIn.Pending)]
    [InlineData(StatusIn.Complete)]
    public void SerializationRoundtrip_Works(StatusIn rawValue)
    {
        // force implicit conversion because Theory can't do that for us
        ApiEnum<string, StatusIn> value = rawValue;

        string json = JsonSerializer.Serialize(value, ModelBase.SerializerOptions);
        var deserialized = JsonSerializer.Deserialize<ApiEnum<string, StatusIn>>(
            json,
            ModelBase.SerializerOptions
        );
