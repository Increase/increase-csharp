...
public class HttpResponse : IDisposable
{
    ...
    /// <summary>
    /// Returns the value of the <c>Idempotent-Replayed</c> header,
    /// or null if there's no such header in the response.
    /// </summary>
    public string? IdempotentReplayed
    {
        get
        {
            return RawMessage.Headers.TryGetValues("Idempotent-Replayed", out var value)
    ...
    public async Task<T> Deserialize<T>(Threading::CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            return await JsonSerializer
                ...
                ?? throw new IncreaseInvalidDataException("Response cannot be null");
        }
