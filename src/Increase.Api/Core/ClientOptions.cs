...
public record struct ClientOptions()
{
    ...
    /// <see cref="IIncreaseClient.WithRawResponse"/>, don't perform validation until deserialization
    /// is triggered.</para>
    /// </summary>
    ...
    Lazy<string> _apiKey = new(() =>
        Environment.GetEnvironmentVariable("INCREASE_API_KEY")
    ...
    Lazy<string?> _webhookSecret = new(() =>
        Environment.GetEnvironmentVariable("INCREASE_WEBHOOK_SECRET")
    );
