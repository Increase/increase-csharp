...
public interface ICardAuthorizationService
{
    ...
    /// can be made. The response will contain either a `pending_transaction` or a
    /// `declined_transaction`; the other attribute will be null. If the authorization
    /// is declined, the reason is available on the Declined Transaction at
    /// `source.card_decline.reason`.
    /// </summary>
