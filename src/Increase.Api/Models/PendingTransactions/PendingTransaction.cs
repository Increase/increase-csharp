...
public sealed record class PendingTransaction : JsonModel
{
    ...
    /// held: it decreases when the merchant reverses part of the authorization.
    /// The amount that settled is available on the resulting Transaction and on
    /// the Card Payment's `state.settled_amount`.
    /// </summary>
...
public sealed record class Source : JsonModel
{
    ...
    /// <summary>
    /// A SEPA Instant Transfer Instruction object. This field will be present in
    /// the JSON response if and only if `category` is equal to `sepa_instant_transfer_instruction`.
    /// </summary>
    public SepaInstantTransferInstruction? SepaInstantTransferInstruction
    {
        ...
        {
            this._rawData.Freeze();
            return this._rawData.GetNullableClass<SepaInstantTransferInstruction>(
                "sepa_instant_transfer_instruction"
            );
        }
        init { this._rawData.Set("sepa_instant_transfer_instruction", value); }
    }
    ...
    public override void Validate()
    {
        ...
        this.SepaInstantTransferInstruction?.Validate();
...
sealed class SourceCategoryConverter : JsonConverter<SourceCategory>
{
    ...
    )
    {
        return JsonSerializer.Deserialize<string>(ref reader, options) switch
        {
            ...
            "sepa_instant_transfer_instruction" => SourceCategory.SepaInstantTransferInstruction,
    ...
    )
    {
        JsonSerializer.Serialize(
            ...
            value switch
            {
                ...
                SourceCategory.UkFasterPaymentSystemTransferInstruction =>
                    "uk_faster_payment_system_transfer_instruction",
                SourceCategory.SepaInstantTransferInstruction =>
                    "sepa_instant_transfer_instruction",
...
/// JSON response if and only if `category` is equal to `sepa_instant_transfer_instruction`.
/// </summary>
[JsonConverter(
    typeof(JsonModelConverter<
        SepaInstantTransferInstruction,
        SepaInstantTransferInstructionFromRaw
    >)
)]
public sealed record class SepaInstantTransferInstruction : JsonModel
{
    /// <summary>
    ...
    /// the transfer's currency. This is always `EUR`.
    /// </summary>
    public required ApiEnum<string, SepaInstantTransferInstructionCurrency> Currency
    {
        get
        {
            this._rawData.Freeze();
            return this._rawData.GetNotNullClass<
                ApiEnum<string, SepaInstantTransferInstructionCurrency>
            >("currency");
        }
        init { this._rawData.Set("currency", value); }
    }
    ...
    public SepaInstantTransferInstruction() { }
...
#pragma warning disable CS8618
    ...
    public SepaInstantTransferInstruction(
        ...
        : base(sepaInstantTransferInstruction) { }
#pragma warning restore CS8618

    public SepaInstantTransferInstruction(IReadOnlyDictionary<string, JsonElement> rawData)
    {
        this._rawData = new(rawData);
    }
...
#pragma warning disable CS8618
    [SetsRequiredMembers]
    SepaInstantTransferInstruction(FrozenDictionary<string, JsonElement> rawData)
    {
        this._rawData = new(rawData);
    }
#pragma warning restore CS8618

    /// <inheritdoc cref="SepaInstantTransferInstructionFromRaw.FromRawUnchecked"/>
    public static SepaInstantTransferInstruction FromRawUnchecked(
        IReadOnlyDictionary<string, JsonElement> rawData
    )
    {
        return new(FrozenDictionary.ToFrozenDictionary(rawData));
...
class SepaInstantTransferInstructionFromRaw : IFromRawJson<SepaInstantTransferInstruction>
{
    /// <inheritdoc/>
    public SepaInstantTransferInstruction FromRawUnchecked(
        IReadOnlyDictionary<string, JsonElement> rawData
    ) => SepaInstantTransferInstruction.FromRawUnchecked(rawData);
}

/// <summary>
/// The [ISO 4217](https://en.wikipedia.org/wiki/ISO_4217) currency code for the transfer's
/// currency. This is always `EUR`.
/// </summary>
[JsonConverter(typeof(SepaInstantTransferInstructionCurrencyConverter))]
public enum SepaInstantTransferInstructionCurrency
{
    /// <summary>
    /// EUR
    /// </summary>
...
sealed class SepaInstantTransferInstructionCurrencyConverter
    : JsonConverter<SepaInstantTransferInstructionCurrency>
{
    public override SepaInstantTransferInstructionCurrency Read(
        ref Utf8JsonReader reader,
        System::Type typeToConvert,
        JsonSerializerOptions options
    )
    {
        return JsonSerializer.Deserialize<string>(ref reader, options) switch
        {
            "EUR" => SepaInstantTransferInstructionCurrency.Eur,
            _ => (SepaInstantTransferInstructionCurrency)(-1),
        };
    }

    public override void Write(
        Utf8JsonWriter writer,
        SepaInstantTransferInstructionCurrency value,
        JsonSerializerOptions options
    )
    {
        JsonSerializer.Serialize(
            ...
            value switch
            {
                SepaInstantTransferInstructionCurrency.Eur => "EUR",
                _ => throw new IncreaseInvalidDataException(
                    string.Format("Invalid value '{0}' in {1}", value, nameof(value))
                ),
            },
