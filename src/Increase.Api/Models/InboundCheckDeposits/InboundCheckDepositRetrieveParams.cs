...
/// </summary>
public record class InboundCheckDepositRetrieveParams : ParamsBase
{
    public string? InboundCheckDepositID { get; init; }

    public InboundCheckDepositRetrieveParams() { }

#pragma warning disable CS8618
    [SetsRequiredMembers]
    public InboundCheckDepositRetrieveParams(
        InboundCheckDepositRetrieveParams inboundCheckDepositRetrieveParams
    )
        : base(inboundCheckDepositRetrieveParams)
    {
...
#pragma warning restore CS8618
    ...
    public static InboundCheckDepositRetrieveParams FromRawUnchecked(
        IReadOnlyDictionary<string, JsonElement> rawHeaderData,
        IReadOnlyDictionary<string, JsonElement> rawQueryData,
        string inboundCheckDepositID
    )
    ...
    public virtual bool Equals(InboundCheckDepositRetrieveParams? other)
    {
        if (other == null)
        {
            return false;
    ...
    public override Uri Url(ClientOptions options)
    {
        return new UriBuilder(
            options.BaseUrl.ToString().TrimEnd('/')
                + string.Format("/inbound_check_deposits/{0}", this.InboundCheckDepositID)
        )
        {
